...
b. get the site name from the raw data object name (e.g. we read in `alegria_mooring_ale_20210617.csv` and saved it to an object named `alegria`)
...
7.  update `select` to that it's flexible
```{r}
#| eval: false
#| echo: true
#| message: false
...
clean_ocean_temps <- function(raw_df, include_temps = c("Temp_top", "Temp_mid", "Temp_bot")){ # 6
  ...
  temps_clean <- raw_df |> 
    select(all_of(all_cols)) |> # ! https://tidyselect.r-lib.org/reference/all_of.html !
    ...
    mutate(month_name = as.factor(month.name[month])) |>
    replace_with_na(replace = list(Temp_bot = 9999, Temp_top = 9999, Temp_mid = 9999)) |> 
    select(any_of(c("site", "date_time", "year", "month", "day", "month_name", "Temp_bot", "Temp_mid", "Temp_top"))) # 7 ! https://tidyselect.r-lib.org/reference/all_of.html !
...
Fifth iteration goal: add if else to evaluate data
