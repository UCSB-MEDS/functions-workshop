...
There are *many* creative ways to go about forming the site name that will get added as a repeating value to the `site` column -- two that came to mind to start were:
...
clean_ocean_temps <- function(raw_data){
  ...
  site_name <- deparse(substitute(raw_data)) # <1>
  site_name <- paste(str_to_title(site_name), "Reef") # <2>
    
  # clean data ----
  temps_clean <- raw_data |> 
    select(year, month, day, decimal_time, Temp_top, Temp_mid, Temp_bot) |>
    filter(year %in% c(2005:2020)) |> 
    mutate(site = rep(site_name)) |> # <3>
    unite(col = date, year, month, day, sep = "-", remove = FALSE) |> 
    mutate(time = times(decimal_time)) |> 
    unite(col = date_time, date, time, sep = " ", remove = TRUE) |> 
    mutate(date_time = as.POSIXct(date_time, "%Y-%m-%d %H:%M:%S", tz = "GMT"),
...
2. format our extracted character string (convert to Title Case and paste `"Reef"` at the end (e.g. `"alegria"` will become `"Alegria Reef"`) ; `r fontawesome::fa("book-open-reader", fill = "#5A5A5A", a11y = "sem")` I referenced [this resource](https://stringr.tidyverse.org/reference/case.html)) 
3. substitute our formatted character string, `site_name` in for the hard-coded site name in `mutate(site = rep("___ Reef"))`
...
2. create a vector of variable names that should *always* be `select`ed
...
#...........................clean data........................... 
alegria_clean <- clean_ocean_temps(raw_data = alegria, include_temps = c("Temp_bot")) # includes only `Temp_bot` # <1>
mohawk_clean <- clean_ocean_temps(raw_data = mohawk) # includes all three temp cols (`Temp_top`, `Temp_mid`, `Temp_bot`) by default # <1>
carpinteria_clean <- clean_ocean_temps(raw_data = carpinteria, include_temps = c("Temp_mid", "Temp_bot")) # includes only `Temp_mid` & `Temp_bot` # <1>
```
...
#...........................clean data...........................  
alegria_clean <- clean_ocean_temps(raw_data = alegria, include_temps = c("Temp_bot")) # <1>
mohawk_clean <- clean_ocean_temps(raw_data = mohawk) # <1>
carpinteria_clean <- clean_ocean_temps(raw_data = carpinteria, include_temps = c("Temp_mid", "Temp_bot")) # <1>
...
plot_ocean_temps <- function(clean_data) { # <1>
  ...
  temps_plot <- clean_data |> # <2>
    ...
    scale_fill_gradientn(colors = c("#2C5374","#778798", "#ADD8E6", "#EF8080", "#8B3A3A"), name = "Temp. (°C)") +
    labs(x = "Bottom Temperature (°C)",
         title = "Bottom Temperatures at ___ Reef, Santa Barbara, CA", # <4>
         subtitle = "Temperatures (°C) aggregated by month from 2005 - 2020") +
    ggridges::theme_ridges(font_size = 13, grid = TRUE) +
    theme(
      axis.title.y = element_blank()
...
4. and add some placeholder text (*___ Reef*) in the `title` field -- we'll make this flexible in our next version
...
#........................source functions........................  
source("utils/clean_ocean_temps.R") # <1>
