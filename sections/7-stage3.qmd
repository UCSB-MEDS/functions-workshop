...
3. split apart the file name at "_" using `stringr::str_split_1()` -- this function takes a single string and returns a character vector (e.g. `"alegria_mooring_ale_20210617.csv"` > `"alegria"      "mooring"      "ale"          "20210617.csv"`)
4. get the first element of the vector, which is the name of the site (i.e. `"alegria"`; `r fontawesome::fa("book-open-reader", fill = "#5A5A5A", a11y = "sem")` I referenced this [resource](https://stringr.tidyverse.org/reference/str_split.html)); this will become the name of the object that our raw data is saved to 
...
```
1. assign the data that you're reading in to the object name you created using the `assign()` function (`r fontawesome::fa("book-open-reader", fill = "#5A5A5A", a11y = "sem")` I referenced [this resource](https://statisticsglobe.com/r-write-read-multiple-csv-files-for-loop))
...
for (i in 1:length(raw_dfs)) {
  ...
  df_name <- raw_dfs[i] # <2>
  df_clean_name <- paste0(df_name, "_clean") # <2>
...
2. get a data frame from our list of `raw_data` and save it to an object called `df_name` (or something intuitive)
...
first_df_name <- raw_dfs[1]

# call an object from environment using the  ----
test <- get(first_df_name) # <1>

# combining the two steps above into one ----
test <- get(raw_dfs[1]) # <2>
```
...
2. combining both the previous steps into one line of code
